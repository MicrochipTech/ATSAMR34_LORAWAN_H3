...
void LORAWAN_Init(AppDataCb_t appdata, JoinResponseCb_t joindata/*,IsmBand_t ismBand*/) // this function resets everything to the default values
{
	RadioError_t status;
	/*    #if SAM0 // this is non-portable code */
	uint8_t pid;
	...
	/*    #endif // this is non-portable code */
...
#ifdef UT
	/*This is added to verify LBT feature in Unit tests*/
	loRa.featuresSupported |= LBT_SUPPORT;
#endif
...
StackRetStatus_t LORAWAN_RxDone(uint8_t *buffer, uint8_t bufferLength)
{
	...
	if (loRa.macStatus.macPause == DISABLED) {
		...
		if ((mhdr.bits.mType == FRAME_TYPE_JOIN_ACCEPT) && (loRa.activationParameters.activationType == 0) && (loRa.lorawanMacStatus.joining == 1)) {
			...
			if (false == isMcastpkt) { // fcntDn validation            
...
static uint8_t* MacExecuteCommands(uint8_t *buffer, uint8_t fOptsLen)
{
	...
	while ((ptr < (buffer + fOptsLen)) && (done == false)) {
		...
		} else { // Invalid payload in MAC cmd will terminate the MAC command processing sequence
			done = true;
			ptr = buffer + fOptsLen;
			loRa.macCommands[loRa.crtMacCmdIndex].receivedCid = INVALID_VALUE;
		}
...
static uint8_t* ExecuteRxTimingSetup(uint8_t *ptr)
{
	...
	if (0 == rfuBits) { /* Process only if RFU bits are non-zero */
...
static void SetReceptionNotOkState(void)
{
	if (CLASS_C == loRa.edClass) { // just drop the frame in class C, wait for timeout to notify application
		loRa.isTransactionDone = true;
