...
typedef enum _JoinNonceType_t
{
    JOIN_NONCE_INCREMENTAL = 0,
    JOIN_NONCE_RANDOM
} JoinNonceType_t;
...
/* Bands enabled at compile time */
    ...
    SEND_LINK_CHECK_CMD,
    /* Returns the type of update used for join nonce */
    JOIN_NONCE_TYPE,
    /* DevNonce */
    DEV_NONCE
} LorawanAttributes_t;
...
typedef union
{
	...
	struct
	{
		...
		unsigned syncronization:1;
		unsigned retransmittedownlink:1;                      //if set, there is no need to send an immediat packet because the application sent one from the callback
	};
} LorawanMacStatus_t;
