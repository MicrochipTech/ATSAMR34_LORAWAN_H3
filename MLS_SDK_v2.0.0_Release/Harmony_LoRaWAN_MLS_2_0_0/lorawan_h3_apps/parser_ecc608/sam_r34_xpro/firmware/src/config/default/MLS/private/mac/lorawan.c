...
StackRetStatus_t LORAWAN_SetAttr(LorawanAttributes_t attrType, void *attrValue)
{
    ...
    if (true == loRa.isTransactionDone)
    {
        ...
        {
            uint8_t alreadyOn = loRa.cryptoDeviceEnabled;
            ...
            if (loRa.cryptoDeviceEnabled)
            {
                ...
                {
                    SAL_CRYPTO_Init();
                    SAL_Read(SAL_DEV_EUI, loRa.activationParameters.deviceEui.buffer);
                    SAL_Read(SAL_JOIN_EUI, loRa.activationParameters.joinEui.buffer);
                }
            }
            else
            {
                ...
                {
                    SAL_CRYPTO_Deinit();
                    loRa.activationParameters.deviceEui.members.genericEuiH = 0;
                    loRa.activationParameters.deviceEui.members.genericEuiL = 0;
                    loRa.activationParameters.joinEui.members.genericEuiH = 0;
                    loRa.activationParameters.joinEui.members.genericEuiL = 0;
                }
            }
            PDS_STORE(PDS_MAC_CRYPTO_DEV_ENABLED);
            result = LORAWAN_SUCCESS;
        }
        break;
        ...
        case DEV_NONCE:
        {
			loRa.devNonce = *(uint16_t *) attrValue ;
			result = LORAWAN_SUCCESS ;            
        }
        break;
        default:
...
StackRetStatus_t LORAWAN_GetAttr(LorawanAttributes_t attrType, void *attrInput, void *attrOutput)
{
    ...
    {
    case CRYPTODEVICE_ENABLED:
        *(uint8_t *)attrOutput = loRa.cryptoDeviceEnabled;
        break;
    ...
    *(uint8_t *)attrOutput = loRa.maxRepetitionsUnconfirmedUplink;
    ...
    case DEV_NONCE:
	{
		*(uint16_t *)attrOutput = (uint16_t)loRa.devNonce ;
	}
	break ;    
    default:
        result = LORAWAN_INVALID_PARAMETER;
