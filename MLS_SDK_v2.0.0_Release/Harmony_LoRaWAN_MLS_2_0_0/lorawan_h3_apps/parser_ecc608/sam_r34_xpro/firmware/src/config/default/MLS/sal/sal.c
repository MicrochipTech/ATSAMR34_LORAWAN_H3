...
static uint8_t nonceIn[32] = {0};

/* Variable array to hold the Key Encryption Key */
static uint8_t keyEncryptionKey[32];
/* Default configuration for an ATECC device on the I2C bus */
...
//#ifdef CRYPTO_DEV_ENABLED	
    if (cryptoDevEnabled)
    {
        ...
        switch(key_type)
        {
            ...
            case SAL_MCAST_NWKS_KEY:
            {
                 ...
                 */
                sal_WriteKeyEncryptionKey();
                atcab_random(&nonceIn[0]);
                status = atcab_read_enc(keyId, block, key, &keyEncryptionKey[0], KEK_SLOT, nonceIn);
    ...
    #else
                //status = atcab_read_bytes_zone(ATCA_ZONE_DATA, keyId, 0, key, SAL_EUI_LEN);
                // Read custom DevEUI in ASCII format and convert into bin.
...
static SalStatus_t sal_WriteKeyEncryptionKey(void)
{
     ...
     * \return ATCA_SUCCESS on success, otherwise an error code.
     */
	atcab_random(keyEncryptionKey);

	if (ATCA_SUCCESS == status)
	{
		/* Write Key Encryption Key of 32 Bytes in KEK_SLOT */
		status = atcab_write_zone(ATCA_ZONE_DATA, KEK_SLOT, 0, 0, &keyEncryptionKey[0], 32);
	}
