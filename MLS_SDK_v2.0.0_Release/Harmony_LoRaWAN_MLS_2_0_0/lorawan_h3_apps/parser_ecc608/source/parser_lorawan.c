...
void Parser_LoraGetCryptoDevEnabled(parserCmdInfo_t* pParserCmdInfo)
{
	...
	pParserCmdInfo->pReplyCmd = (char*) gapParseOnOff[cryptoDevEnabled ];
}
...
{
	JoinNonceType_t jntype = (JoinNonceType_t) atoi(pParserCmdInfo->pParam1);
	StackRetStatus_t status = LORAWAN_INVALID_PARAMETER;
...
void Parser_LoraSetDevNonce(parserCmdInfo_t* pParserCmdInfo)
{
	...
	// Parameter validation
	if(Validate_Uint16DecAsciiValue(pParserCmdInfo->pParam1, &devNonce))
	{
		status = LORAWAN_SetAttr(DEV_NONCE, &devNonce);
	}
...
{
	uint16_t devNonce ;
	LORAWAN_GetAttr(DEV_NONCE, NULL, &devNonce) ;
	utoa(devNonce, aParserData, 10) ;
	pParserCmdInfo->pReplyCmd = aParserData;
}
